%% =========================================================
%  step2_generate_pv_actual.m  ―  PV実績出力データの生成（導入量補正付き）
%  =========================================================
%
%  【役割】
...
%
%  【実行方法】
%    >> step2_generate_pv_actual(2018)   % 2018年度分を生成
%    >> step2_generate_pv_actual(2019)   % 2019年度分を生成
%
...
function step2_generate_pv_actual(year)

%% --- データ読み込み（相対パス: input_data フォルダ） ---
load(fullfile('input_data', ['PV_capa_',num2str(year),'.mat']))      % 変数: PV_capa      → 各月の実際のPV導入容量 [MW]
load(fullfile('input_data', ['Pv_real_out_',num2str(year),'.mat']))  % 変数: Pv_real_out  → 補正前のPV実績出力（1440点/日）
...
%% --- 出力ファイルへ保存 ---
save(fullfile('output', ['PV_',num2str(year),'.mat']), 'data_all')
% → 保存先: output/PV_YYYY.mat（変数名: data_all、各行が1日分・48列）
