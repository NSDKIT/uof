...
### 1.2. 前処理の実行（.mat ファイルが存在しない場合）

`model.m` を実行した際に「必須入力ファイルが見つかりません」というエラーが表示された場合は、以下の前処理スクリプトを実行して、`基本データ/` に `.mat` ファイルを生成する必要があります。

**各スクリプトは、それぞれのフォルダに移動してから実行してください。**

| 目的 | 実行するスクリプト | 必要なもの |
| :--- | :--- | :--- |
| **需要データ作成** | `需要実績・予測作成/make_load.m` | `(譲渡データ) 需給データ 北陸電力.xlsx` |
| **予測PV出力作成** | `予測PV出力作成/make_PVF_year_for_agc.m` | 気象庁のMSM気象モデルの `.bin` ファイル |
| **実績PV出力作成** | `PV実出力作成/new_get_PV300_day_for_agc.m` | `ido_keido.xlsx`, `short_f.mat` |

> **注意**: これらの前処理スクリプトは、特定のExcelファイルや、外部からダウンロードする気象データに依存しているため、完全な自動実行はできません。詳細は各スクリプトのコメントを参照してください。
