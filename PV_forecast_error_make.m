...
%% --- データ読み込み ---
load(['PV_base_',num2str(year),'.mat'])      % 変数: PV_base → 基準PV導入量（月別）
load(['PV_forecast_',num2str(year),'.mat'])  % 変数: data_all → PV予測出力 [MW]
PV_f = data_all(:, 1:48);                    % 30分×48コマ分のみ抽出（予測値）

load(['PV_',num2str(year),'.mat'])           % 変数: data_all → PV実績出力 [MW]
PV_o = data_all;                             % 実績値

load(['Load_',num2str(year),'.mat'])         % 変数: data_all → 電力需要 [MW]
Lo = data_all;                               % 負荷データ
...
% → 保存先: ERROR_YYYY.mat（変数名: ERROR、サイズ: [日数 × 50]）
